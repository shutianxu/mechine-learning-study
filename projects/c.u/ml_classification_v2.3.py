...
data = pd.read_csv("C:/Users/1707500/Desktop/cusc_data_111.csv", sep = '\t',encoding = 'gb2312',names = header)

# =============================================================================
# print(df)
# =============================================================================

data = data.drop(["device_number","didi_cnt","didi_dura","autohome_cnt_201711","autohome_dura_201711","gongxiangqiche_cnt","gongxiangqiche_dura","gas_vis_cnt","xiandai_dealer_cnt","daohang_cnt","gongxiangdanche_cnt","autohome_cnt_201712"],axis=1)
...
clf = RandomForestClassifier(n_estimators=100, max_depth=60 ,min_samples_split=1000, random_state=42,class_weight="balanced")
        ...
        if ((i%5 == 0) and i != 0):
            for k in range(2001):
                if ((k%1000 ==0) and (k != 0)):
                    ...
                    with open('D:/document/crawling/random_forest_vis_dealer_new.txt','a') as f: # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！                        
                        f.write('\n')
                        f.write('\n')
                        f.write("Index: %d" %num)
                        f.write('\n')
